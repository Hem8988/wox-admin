	...
	public function addSubject(Request $request)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			{	
				...
				{
					return Redirect::to('/subjects')->with('success', 'Subject'.Config::get('constants.added'));	
				}
			}
			else
			{
	...
	public function editSubject(Request $request, $id = NULL)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			if($count  == 0)
			{
				...
				{
					return Redirect::to('/subjects')->with('success', 'Subject'.Config::get('constants.edited'));
				}
			}
			else
			{
		...
		{	
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/subjects')->with('error', 'Subject'.Config::get('constants.not_exist'));
				}	
			}
			else
			{
				return Redirect::to('/subjects')->with('error', Config::get('constants.unauthorized'));
			}		
		}				
	}
	...
	public function viewSubject(Request $request, $id)
	{
		if(isset($id) && !empty($id))
		{
			...
			else
			{
				...
				{
					return Redirect::to('/linked_subjects')->with('error', 'Subject'.Config::get('constants.not_exist'));
				}
				else
				{
		...
		else
		{
			...
			{
				return Redirect::to('/linked_subjects')->with('error', Config::get('constants.unauthorized'));
			}
			else
			{
				return Redirect::to('/dashboard')->with('error', Config::get('constants.unauthorized'));
