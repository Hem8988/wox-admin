...
class TestController extends Controller
{
	...
	public function addTest(Request $request)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/tests')->with('success', 'Test'.Config::get('constants.added'));
			}
		}
	...
	public function editTest(Request $request, $id = NULL)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/tests')->with('success', 'Your Test has been edited successfully.');
			}		
		}
		else
		{	
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/tests')->with('error', 'Test Id does not exist.');
				}	
			}
			else
			{
				return Redirect::to('/tests')->with('error', Config::get('constants.unauthorized'));
			}		
		}				
	}
	...
	public function viewTest(Request $request, $id)
	{
		if(isset($id) && !empty($id))
		{
			...
			else
			{
				...
				{
					return Redirect::to('/tests')->with('error', 'Test'.Config::get('constants.not_exist'));
				}
				else if(Auth::user()->role == 2) //for vendors
				{
					return Redirect::to('/assigned_tests')->with('error', 'Test'.Config::get('constants.not_exist'));
				}
				else
				{
		...
		else
		{
			...
			{
				return Redirect::to('/assigned_tests')->with('error', Config::get('constants.unauthorized'));
			}
			else
			{
				return Redirect::to('/dashboard')->with('error', Config::get('constants.unauthorized'));
				...
				if(ScheduledTest::where('id', '=', $id)->exists()) 
				{
					...
					if($role == 1 || $role == 2) //for superadmin and vendor
					{
						...
						if($type == 'test_submitted_copy')
						{
							...
							{
								return Redirect::to('/dashboard')->with('error', 'PDF'.Config::get('constants.not_exist'));
							}		
						}
						...
						{
							return redirect('/dashboard')->with('error', Config::get('constants.unauthorized'));
						}	
					}
					else
					{
	...
	public function viewScheduledTest(Request $request, $id)
	{
		...
		if(isset($id) && !empty($id))
		{
			...
			{
				return Redirect::to('/scheduled_tests')->with('error', 'Test Id does not exist.');
			}
		}
		else
		{
			return Redirect::to('/scheduled_tests')->with('error', Config::get('constants.unauthorized'));
		}
	}
