...
class ManageholidaypackageController extends Controller
{
	...
	public function store(Request $request) 
	{
		...
		if ($request->isMethod('post')) 
		{
			$this->validate($request, [
									  ...
									  $depPrice = $request->has('dep_price') ? json_decode($request->dep_price,true) : Null;
			...
			if(!empty($requestData['dep_price'])){
				if(count( $depPrice) > 0){
					...
					$pprice->package_id = $obj->id;
						$pprice->departure_date = $depPrice['departure_date'];
						$pprice->no_of_seats = $depPrice['seats'];
						$pprice->twin = $depPrice['twin'];
						$pprice->triple = $depPrice['triple'];
						$pprice->single = $depPrice['single'];
						$pprice->child_with_bed = $depPrice['child_with_bed'];
						$pprice->child_without_bedbelow12 = $depPrice['child_without_bedbelow12'];
						$pprice->child_without_bedbelow26 = $depPrice['child_without_bedbelow26'];
						$pprice->infant = $depPrice['infant'];
						$pprice->adult_flight = $depPrice['adult_flight'];
						$pprice->child_flight = $depPrice['child_flight'];
						$pprice->infant_flight = $depPrice['infant_flight'];
						$pprice->price_type = $depPrice['price_type'];
						$pprice->booking_amt = $depPrice['booking_amt'];
						$pprice->dis_type = $depPrice['dis_type'];
						$pprice->bal_rec_day = $depPrice['balance_rec'];
						$pprice->flight_type	= $depPrice['flight_type'];
							if($depPrice['flight_type'] == 1){
								$pprice->flightname	= $depPrice['flightname']; 
								$pprice->dep_city	= $depPrice['dep_city']; 
								$pprice->arv_city	= $depPrice['arv_city']; 
								$pprice->dep_time	= $depPrice['dep_time']; 
								$pprice->arv_time	= $depPrice['arv_time']; 
							}
		...
		if(!empty($requestData['all_hotel_name'])){
			...
			{
				return Redirect::to('/holidaypackage/edit/'.base64_encode(convert_uuencode($obj->id)))->with('success', 'Package added Successfully');
			}				
		}		
	}  
	...
	public function storeDuplicate(Request $request) 
	{
		...
		if(!empty($requestData['all_hotel_name'])){
			...
			{
				return Redirect::to('/holidaypackage')->with('success', 'Package added Successfully');
			}				
		}	
	}  
	...
	{			
		...
		if ($request->isMethod('post')) 
		{
			...
			$requestData 		= 	$request->all();
		     ...
		     if($request->has('dep_price')){
				if(is_array($request->dep_price)){
					$depPrice = $request->has('dep_price') ? json_decode($request->dep_price[0],true) : Null;
					$depId = $depPrice['priceid'];
				}else{
					$depPrice = $request->has('dep_price') ? json_decode($request->dep_price,true) : Null;
			...
			$aa = array();
			// $dep_price = $request->dep_price[0];
			if(!empty( $request->dep_price)){
				if(count($depPrice) > 0){
					
					foreach($depPrice as $key=>$ppriced){
						$queryArrays = array();
						if($key == 'priceid'){
			...
			if(!empty($requestData['dep_price'])){
				...
				if(count($depPrice) > 0){
					// for($ipth = 0; $ipth<count($dep_price); $ipth++){
						...
						$isexist = PackagePrice::where('id',$depId)->exists();
						if($isexist){
							$pprice =  PackagePrice::find($depId);
							$pprice->departure_date = $depPrice['departure_date'];
							$pprice->no_of_seats = $depPrice['seats'] ?? $depPrice['no_of_seats'];
							$pprice->twin = $depPrice['twin'];
							$pprice->triple = $depPrice['triple'];
							$pprice->single = $depPrice['single'];
							$pprice->child_with_bed = $depPrice['child_with_bed'];
							...
							$pprice->booking_amt = $depPrice['booking_amt'];
							$pprice->dis_type = $depPrice['dis_type'];
							$pprice->bal_rec_day = $depPrice['balance_rec'] ?? Null;
							$pprice->flight_type	=	$depPrice['flight_type'];
							if($depPrice['flight_type'] == 1){
								$depPrice['dep_city']; 
								$pprice->flightname	=	$depPrice['flightname']; 
							...
							PackagePrice::where('package_id',$requestData['id'])->delete();
							$pprice = new PackagePrice;
							$pprice->package_id = $obj->id;
							$pprice->departure_date = $depPrice['departure_date'];
							$pprice->no_of_seats = $depPrice['seats'];
			...
			{
				return Redirect::to('/holidaypackage/edit/'.base64_encode(convert_uuencode($requestData['id'])))->with('success', 'Package Edited Successfully');
			}			
			...
			if(isset($id) && !empty($id)) 
			{
				...
				{
					return Redirect::to('/holidaypackage')->with('error', 'Package Not Exist');
				}	
			}
			else
			{
				return Redirect::to('/holidaypackage')->with('error', Config::get('constants.unauthorized'));
			}		
		}				
	}
	...
	public function addSort(Request $request){
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/sort-package')->with('success', 'Package Sorted Successfully');
			}
		}
	}
