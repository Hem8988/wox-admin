...
class LeadController extends Controller
{
		...
		if(isset($id) && !empty($id)) 
			{
				...
				{
					return Redirect::to('/leads')->with('error', 'Lead Not Exist');
				}	
			}
			else
			{
				return Redirect::to('/leads')->with('error', Config::get('constants.unauthorized'));
			}
	}
	 ...
	 public function store(Request $request)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			{ 
				return Redirect::to('/leads')->with('success', 'Lead added Successfully');
			} 				
		}	
	} 
	...
	//check authorization start	
		...
		if ($request->isMethod('post')) 
		{
			...
			{ 
				return Redirect::to('/leads/edit'.base64_encode(convert_uuencode(@$requestData['id'])))->with('success', 'Lead updated Successfully');
			}				
		}
		else
		{	
