...
class AdminController extends Controller
{
	...
	public function edittaxrates(Request $request, $id = Null){
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/settings/taxes/taxrates/edit/'.base64_encode(convert_uuencode(@$requestData['id'])))->with('success', 'Tax updated Successfully');
			}				
		}
		else
		{	
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/settings/taxes/taxrates')->with('error', 'Tax Not Exist');
				}	
			}
			else
			{
				return Redirect::to('/settings/taxes/taxrates')->with('error', Config::get('constants.unauthorized'));
			}		
		}
	}
	...
	public function savetaxrate(Request $request){
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/settings/taxes/taxrates/edit/'.base64_encode(convert_uuencode(@$obj->id)))->with('success', 'Tax added Successfully');
			}				
		}
	}
	...
	public function editSeo(Request $request, $id = NULL)
	{
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/dashboard')->with('success', 'SEO Information for this Page'.Config::get('constants.edited'));
			}				
		}
		else
		{	
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/dashboard')->with('error', 'Page'.Config::get('constants.not_exist'));
				}	
			}
			else
			{
		...
		if ($request->isMethod('post')) 
		{
			...
			if($role == 1 || $role == 7)
			{
				...
				$url = $link = \URL::to('/').'/agent/login';
					...
					$this->send_email_template($replace, $replace_with, 'account-approved', @$userData->email,'ZAP Booking Account Approved',$set->primary_email);
