...
if ($request->has('cityid')) 
			 ...
			 ->addColumn('action', function ($cities) {
				 return '<div class="nav-item dropdown action_dropdown">
										...
										<div class="dropdown-menu">
											<a href="'.URL::to('/cities/edit/'.base64_encode(convert_uuencode(@$cities->id))).'"><i class="fa fa-edit"></i> Edit</a>
	...
	{			
		...
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/cities')->with('success', 'City Edited Successfully');
			}				
		}
		else
		{	
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/cities')->with('error', 'City Not Exist');
				}	
			}
			else
			{
				return Redirect::to('/cities')->with('error', Config::get('constants.unauthorized'));
			}		
		}				
	}
