...
class ReviewTestController extends Controller
{
	...
	public function viewReviewTest(Request $request, $id)
	{
		if(isset($id) && !empty($id))
		{
			...
			{
				return Redirect::to('/review_tests')->with('error', 'Test does not exist.');
			}
		}
		else
		{
			return Redirect::to('/review_tests')->with('error', Config::get('constants.unauthorized'));
		}
	}
	...
	public function submitReview(Request $request)
	{
		if ($request->isMethod('post')) 
		{
			...
			else
			{
				...
				return Redirect::to('/view_review_test/'.$this->encodeString($requestData['id']))->with('success', 'This Test has been reviewed successfully.');
			}
		}	
	}
	...
	public function deleteStudentCopy(Request $request)
	{
		if ($request->isMethod('post')) 
		{
			...
			{	
				...
				{
					return Redirect::to('/view_review_test/'.$this->encodeString($requestData['id']))->with('success', 'Student Copy has been deleted successfully. Now Student will submit copy.');
				}
			}
			else
			{
				return Redirect::to('/review_tests')->with('error', 'Schedule Test does not exist.');
			}		
		}	
	}
	...
	public function deleteReviewCopy(Request $request)
	{
		if ($request->isMethod('post')) 
		{
			...
			{	
				...
				{
					return Redirect::to('/view_review_test/'.$this->encodeString($requestData['id']))->with('success', 'Review Copy has been deleted successfully. Now you can review again.');
				}
			}
			else
			{
