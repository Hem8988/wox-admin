...
class PackageController extends Controller
{
	...
	public function packbooking(Request $request, $packageid = NULL)
    {
		if(isset($packageid) && !empty($packageid))
		{
			...
			$allprice = \App\PackagePrice::select('departure_date','twin','single','triple','child_with_bed','child_without_bedbelow12','child_without_bedbelow26','infant','no_of_seats')->where('package_id',$id)->whereRaw('json_contains(departure_date, \'["' . $seldate . '"]\')')->first();
			...
			$selectedseats = $adults + $Infant + $cwb + $cwob + $cwobb;
				$exp = json_decode($allprice->departure_date);
				 $no_of_seats = json_decode($allprice->no_of_seats);
				 foreach($exp as $key => $lprice){
					...
					$yearsel = explode('/',$seldate);
					$datesel = $yearsel[2].'-'.($yearsel[0]).'-'.$yearsel[1];
					$selmdate = date('Y-m-d', strtotime($datesel));
					 if(strtotime($selmdate) == strtotime($mdate)){
						 $seats = $no_of_seats[$key];
	...
	public function payment(Request $request){
		if($request->payment_method != ''){
			...
			$date = $explode[1].'/'.$explode[2].'/'.$explode[0];
			 $allprice = PackagePrice::select('twin','single','triple','child_with_bed','child_without_bedbelow12','child_without_bedbelow26','infant')->where('package_id',$id)->whereRaw('json_contains(departure_date, \'["' . $date . '"]\')')->first();
	...
	$api = new Api('rzp_test_9BsnxNsU16jr90', 'BxykBD4klu8Zz5l15IcHSZwl');
        ...
        if(count($input)  && !empty($input['razorpay_payment_id'])) {
			...
			$seldate = $selecteddate[1].'/'.$selecteddate[2].'/'.$selecteddate[0];
			$allprice = \App\PackagePrice::where('package_id',$bookingdetail->package_id)->whereRaw('json_contains(departure_date, \'["' . $seldate . '"]\')')->first();
			//echo '<pre>'; print_r($allprice); die;
			if($allprice){
				 ...
				 foreach($exp as $key => $lprice){
					...
					$selmdate = date('Y-m-d', strtotime($datesel));
					 if(strtotime($selmdate) == strtotime($mdate)){
						 $seats = $no_of_seats[$key];
						 if($selectedseats < $seats){
							 $no_of_seats[$key] = $no_of_seats[$key] - $counttravler;
						 }
					 }
				 }
				 
		 	 $noofseats = json_encode($no_of_seats); 
