	...
	public function email(Request $request, $id = Null){
		if ($request->isMethod('post')) 
		{
			...
			if($issuccess){
				...
				$followupsaved				=	$objf->save(); 
				return Redirect::to('/invoice/lists/'.base64_encode(convert_uuencode(@$requestData['id'])))->with('success', 'Invoice send Successfully');
			}else{
				return Redirect::to('/invoice/lists'.base64_encode(convert_uuencode(@$requestData['id'])))->with('error', 'Please try again');
			}
		}else{
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/invoice')->with('error', 'invoice Not Exist');
				}		
			}else{
				return Redirect::to('/invoice')->with('error', Config::get('constants.unauthorized'));
			}
		}
	}
	...
	{		
		...
		{ 
			...
			if(@$requestData['save_type'] == 'save_send'){
				return Redirect::to('/invoice/email/'.base64_encode(convert_uuencode(@$obj->id)))->with('success', 'Invoice saved Successfully');
			}else if(@$requestData['save_type'] == 'save_print'){
				return Redirect::to('/invoice/lists/'.base64_encode(convert_uuencode(@$obj->id)).'?print_invoice=true')->with('success', 'Invoice saved Successfully');
			}else if(@$requestData['save_type'] == 'save_share'){
				return Redirect::to('/invoice/lists/'.base64_encode(convert_uuencode(@$obj->id)).'?share_invoice=true')->with('success', 'Invoice saved Successfully');
			}
			else{
				return Redirect::to('/invoice/lists'.base64_encode(convert_uuencode(@$obj->id)))->with('success', 'Invoice saved Successfully');
			}
		} 
	} 
		...
		if ($request->isMethod('post')) 
		{
			...
			$invoicedetail = Invoice::where('id', '=', $request->id)->where('user_id', '=', Auth::user()->id)->first();
				...
				if($subtotal < $amount_rec){
					return Redirect::to('/invoice/edit/'.base64_encode(convert_uuencode(@$request->id)))->with('error', 'The payment entered is more than the total amount due for this invoice. Please check and retry.');
				}else{
					$subtotal = 0;
					$total_value = 0;
					$subtotal1 = 0;
            ...
            unlink($array['file']);*/
			if(@$requestData['save_type'] == 'save_send'){
				return Redirect::to('/invoice/email/'.base64_encode(convert_uuencode(@$request->id)))->with('success', 'Invoice updated Successfully');
			}else if(@$requestData['save_type'] == 'save_print'){
				return Redirect::to('/invoice/lists/'.base64_encode(convert_uuencode(@$request->id)).'?print_invoice=true')->with('success', 'Invoice saved Successfully');
			}else if(@$requestData['save_type'] == 'save_share'){
				return Redirect::to('/invoice/lists/'.base64_encode(convert_uuencode(@$request->id)).'?share_invoice=true')->with('success', 'Invoice saved Successfully');
			}
			else{
				return Redirect::to('/invoice/lists/'.base64_encode(convert_uuencode(@$request->id)))->with('success', 'Invoice updated Successfully');
			}
