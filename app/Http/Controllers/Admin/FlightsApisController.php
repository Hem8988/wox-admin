...
class FlightsApisController extends Controller
{
    /**
	...
	public function index(Request $request)
	{
		...
		//check authorization end
		 ...
		 $query 		= ApiDetail::where('api_details_id', '!=', '');
		
		$totalData 	= $query->count();	//for all data
		$lists		= $query->orderby('api_details_id','desc')->get();
		return view('Admin.flights.indexapis',compact(['lists', 'totalData']));	
	}
	...
	public function store(Request $request)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			if(Flight::where('code', @$requestData['code'])->exists()){
				return Redirect::to('/flights/create')->with('error', 'Flights is already exists');
			}
			...
			{
				return Redirect::to('/flights')->with('success', 'Flights added Successfully');
			}
		}			
	} 
	...
	{	
		...
		if ($request->isMethod('post')) 
		{
			...
			if($requestData['code'] != @$fdetail->code){
				if(Flight::where('code', @$requestData['code'])->exists()){
					return Redirect::to('/flights/edit'.base64_encode(convert_uuencode(@$requestData['id'])))->with('error', 'Flights is already exists');
				}
			}
			...
			{
				return Redirect::to('/flights')->with('success', 'Flight'.Config::get('constants.edited'));
			}				
		}
		else
		{	
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/flights')->with('error', 'Flight'.Config::get('constants.not_exist'));
				}	
			}
			else
			{
				return Redirect::to('/flights')->with('error', Config::get('constants.unauthorized'));
			}		
		}				
	}
	...
	public function FlightDetailindexStore(Request $request){
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/flight-detail')->with('success', 'Flights added Successfully');
			}
		}
	}
	...
	public function FlightDetailindexClone(Request $request, $id = NULL){
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/flight-detail')->with('success', 'Flight'.Config::get('constants.edited'));
			}				
		}
		else
		{	
	...
	$users = User::orderby('company_name','ASC')->get();
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/flight-detail')->with('error', 'Flight'.Config::get('constants.not_exist'));
				}	
			}
			else
			{
				return Redirect::to('/flight-detail')->with('error', Config::get('constants.unauthorized'));
			}		
		}	
	}
