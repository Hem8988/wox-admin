...
class ManagetopinclusionController extends Controller
{
	...
	public function store(Request $request)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/topinclusion')->with('success', 'Topinclusion Added Successfully');
			}				
		}	
	...
	{			
		...
		if ($request->isMethod('post')) 
		{
			...
			if(Topinclusion::where('top_inc_id', '=', $requestData['id'])->where('user_id', '=', Auth::user()->id)->exists()) 
			{
				...
				{
					return Redirect::to('/topinclusion')->with('success', 'Top Inclusion Edited Successfully');
				}
		...
		{	
			if(isset($id) && !empty($id))
			{
				...
				{
					return Redirect::to('/topinclusion')->with('error', 'Topinclusion Not Exist');
				}	
			}
			else
			{
				return Redirect::to('/topinclusion')->with('error', Config::get('constants.unauthorized'));
			}		
		}				
	}
