...
class ManageholidaytypeController extends Controller
{
	...
	/* public function store(Request $request)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			{
				return Redirect::to('/holidaytype')->with('success', 'Holidaytype added Successfully');
			}				
		}	
	...
	{			
		...
		if ($request->isMethod('post')) 
		{
			...
			if(Holidaytype::where('theme_id', '=', $requestData['id'])->where('user_id', '=', Auth::user()->id)->exists()) 
			{
				...
				{
					return Redirect::to('/holidaytype')->with('success', 'Holidaytype Edited Successfully');
				} 
			}else{
				if($request->hasfile('image')) 
				{	
		...
		{	
			if(isset($id) && !empty($id)) 
			{
				...
				{
					return Redirect::to('/holidaytype')->with('error', 'Holidaytype Not Exist');
				}	
			}
			else
			{
				return Redirect::to('/holidaytype')->with('error', Config::get('constants.unauthorized'));
			}		
		}				
	}
