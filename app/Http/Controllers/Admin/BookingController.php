			...
			if(BookingDetail::where('id', '=', $id)->exists()) 
				{
					...
					// dd(	$fetchedData);
					// dd(json_decode($fetchedData->booking_response));
					return view('Admin.bookings.detail', compact(['fetchedData']));
				}
				else
				{
					return Redirect::to('/bookings')->with('error', 'Not Exist');
				}
		}else
			{
				return Redirect::to('/bookings')->with('error', Config::get('constants.unauthorized'));
			}
	}
	...
	public function hotelBookingDetail(Request $request, $id = Null){
		if(isset($id) && !empty($id)) 
		{
			...
			if(HotelBookingDetail::where('id', '=', $id)->exists()) 
				...
				{
					return Redirect::to('/bookings?btype=hotel&type=b2c')->with('error', 'Not Exist');
				}
		}else
			{
				return Redirect::to('/bookings?btype=hotel&type=b2c')->with('error', Config::get('constants.unauthorized'));
			}
	}
