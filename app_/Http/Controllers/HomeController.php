...
class HomeController extends Controller
{
    ...
    {
		$emailContent = 'sdsdddssdsd';
		$subject = 'test';
		//Mail::to('pankaj95.mca10.lgc@gmail.com')->send(new CommonMail($emailContent, $subject, 'nishant@zapbooking.com'));
    ...
    {
		$contents = HomePage::where('status','1')->orderBy('created_at','desc')->take(3)->get();
		$banners = Banner::where('status','1')->orderBy('created_at','desc')->take(3)->get();
		$cheaps = CheapFlight::where('status','1')->orderBy('created_at','desc')->take(6)->get();
		//$auth = $this->GetAuthenticate();
		return view('index',compact('contents','banners','cheaps'));
    }
	...
	public function package(Request $request){
		...
		/*Domestic Tour*/
		$queryd = Location::where('dest_type', '=', 'domestic')->where('is_active', '=', '1');	
		$domesticresponse 	= 	$queryd->has('mypackage','>', 0)->with(['mypackage','desmedia'])->sortable(['id'=>'DESC'])->paginate(10);
		/*Domestic Tour*/
		/*International Tour*/
		$queryi = Location::where('dest_type', '=', 'international')->where('is_active', '=', '1');
		$internationalesponse 	= 	$queryi->has('mypackage','>', 0)->with(['mypackage','desmedia'])->sortable(['id'=>'DESC'])->paginate(10);

		// dd($internationalesponse);
		/*International Tour*/
		/*Popular Tour*/
		$queryp = $Packages = Package::where('is_popular', '=', 1)->with(['media','packloc']);
		$co = $queryp->count();
		$populartours = $queryp->paginate(12);	
		/*Popular Tour*/
	...
	public function bkConfiration(){
		if(auth()->guard('agents')->user()){
			$booking = BookingDetail::where('agent_id',auth()->guard('agents')->user()->id)->latest()->first();
		}else{
		$booking = BookingDetail::where('user_id',auth()->user()->id)->latest()->first();
		}
		$flight= json_decode($booking->booking_response,true);
		$flights =bookingsDetails($flight);
	
		// dd($flights);
       return view('booking-confirmation',compact('flights'));
	}
	public function bkError(){
		return view('booking-error');
	}
	public function bkSuccess(){
		...
		$flight = json_decode($booking->booking_response,true);
		//  dd($flight["flight_details"]["data"]["associatedRecords"][0]["reference"]);
		return view('booking-success',compact('flight','booking'));
	}
}
